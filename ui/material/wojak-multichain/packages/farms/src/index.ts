...
const supportedChainId = [ChainId.DOGECHAIN]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet
      ? masterChefAddresses[ChainId.BSC_TESTNET]
      : masterChefAddresses[ChainId.DOGECHAIN]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.DOGECHAIN].includes(chainId),
  }
}
