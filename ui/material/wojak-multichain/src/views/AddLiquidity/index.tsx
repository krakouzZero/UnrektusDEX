...
import { WOJK, USDC } from '@pancakeswap/tokens'
...
export default function AddLiquidity() {
  ...
  const [currencyIdA, currencyIdB] = router.query.currency || [
    native.symbol,
    WOJK[chainId]?.address ?? USDC[chainId]?.address,
  ]
  ...
  async function onZapIn() {
    ...
    if (rebalancing) {
      ...
      if (currencyA?.isNative || currencyB?.isNative) {
        const tokenBIsDOGE = currencyB?.isNative
        method = 'zapInDOGERebalancing'
        args = [
          wrappedCurrency(currencies[tokenBIsDOGE ? Field.CURRENCY_A : Field.CURRENCY_B], chainId).address, // token1
          parsedAmounts[tokenBIsDOGE ? Field.CURRENCY_A : Field.CURRENCY_B].quotient.toString(), // token1AmountIn
          pair.liquidityToken.address, // lp
          maxAmountIn, // tokenAmountInMax
          minAmountOut, // tokenAmountOutMin
          zapIn.zapInEstimated.isToken0Sold && !tokenBIsDOGE, // isToken0Sold
        ]
        value = parsedAmounts[tokenBIsDOGE ? Field.CURRENCY_B : Field.CURRENCY_A].quotient.toString()
      } else {
      ...
      summary = `Zap in ${amount} DOGE for ${symbol}`
      translatableSummary = {
        text: 'Zap in %amount% DOGE for %symbol%',
        data: { amount, symbol },
      }
