...
import { ADMINS, WOJAK_SPACE, SNAPSHOT_VERSION } from './config'
...
export const getVotingPower = async (
  ...
  const [total] = await getScores(WOJAK_SPACE, STRATEGIES, NETWORK, [account], blockNumber)
...
export async function getVotingPowerByCakeStrategy(voters: string[], blockNumber: number) {
  const strategyResponse = await getScores(WOJAK_SPACE, STRATEGIES, NETWORK, voters, blockNumber)
