...
const fetchVaultUserV1 = async (account: string) => {
  ...
  try {
    ...
    return {
      ...
      wojkAtLastUserAction: new BigNumber(userContractResponse.wojkAtLastUserAction.toString()).toJSON(),
    }
  } catch (error) {
    return {
      isLoading: true,
...
const transformData = ({
  ...
  userData: { isLoading, userShares, wojkAtLastUserAction, lastDepositedTime, lastUserActionTime },
}) => {
  return {
    ...
    userData: {
      ...
      wojkAtLastUserAction: new BigNumber(wojkAtLastUserAction),
      lastDepositedTime,
      lastUserActionTime,
    },
  }
